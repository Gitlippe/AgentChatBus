...
async def handle_bus_connect(db, arguments):
    ...
    # ── 组装返回 ──
    agent_payload = {"agent_id": agent.id, "name": agent.name, "registered": True, "token": agent.token}
...
test_bus_connect_no_reuse_agent	在同一连接调用两次，确认不共享 ID，生成各自独立的注册身份
运行命令：
